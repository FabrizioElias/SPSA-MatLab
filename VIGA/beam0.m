...
if mi<=milim
    Astrac=0.8*qsi*VIGAin.b*VIGAin.dinf(sec)*VIGAin.sigmafcd/VIGAin.fyd;
    Ascomp=0;
else
    Astrac=(VIGAin.b*VIGAin.dinf(sec)*VIGAin.sigmafcd/VIGAin.fyd*(0.8*qsilim+(mi-milim)/(1-delta)));
    Ascomp=VIGAin.b*VIGAin.dinf(sec)*VIGAin.sigmafcd*(mi-milim)/((1-delta)*VIGAin.fyd);   
end
