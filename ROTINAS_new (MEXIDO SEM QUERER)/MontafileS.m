function  MontafileS(GEOM, CARGA, DADOS)
...
[fidtpl,msg] = fopen(DADOS.filetpl, 'r'); %Abre o arquivo template para ser lido
[fidsim,msg] = fopen(DADOS.filesim, 'w'); %Abre o arquivo de simulação para escrever nele
...
while ~feof(fidtpl)  %Loop até o final do arquivo template
    ...
    dolar1=findstr(str,'&cargaA'); %VIGAS:peso próprio + parede
    dolar2=findstr(str,'&cargaB'); %PILAR1:Peso próprio
    dolar3=findstr(str,'&cargaC'); %PILAR2:Peso próprio
    dolar4=findstr(str,'&cargaD'); %PILAR2:Carga de vento(sentido contrário ao eixo x)

    %Procura o caractere &geometria() e atribui sua posição à variável dolar:
    dolar101=findstr(str,'&geometriaA'); %VIGA1   - MATERIAL 1 
    dolar102=findstr(str,'&geometriaB'); %PILAR 1 - MATERIAL 2
    dolar103=findstr(str,'&geometriaC'); %PILAR 2 - MATERIAL 3
