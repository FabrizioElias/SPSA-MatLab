...
%FAB - Troca por uma função mais rápida.
%n = prod(size(x));
n = numel(x);
numberOfVariables = n;
...
case 'iter'
   prnt = 3;
% Modificacao do Arquivo Original para guardar informacoes do percurso do
% algoritmo: 11/04/2006
   [arq_iter, msg] = fopen('iteracoes.log', 'w');
    if (arq_iter == -1)
      error(msg);
    end
   [arq_opt, msg] = fopen('opt_simplex.log', 'w');
    if (arq_opt == -1)
      error(msg);
    end
...
usual_delta = -1/3;             % 5 percent deltas for non-zero terms
zero_term_delta = 0.25; %0.00025;      % Even smaller delta for zero elements of x
for j = 1:n
   y = xin;
   if y(j) ~= 0
...
% Modificacao do Arquivo Original para guardar informacoes do percurso do
% algoritmo: 11/04/2006
   fprintf(arq_opt,header);
   fprintf(arq_opt,'\n %5.0f        %5.0f     %12.6g         %s\n', itercount, func_evals, fv(1), how);
   fprintf(arq_iter,' Iteration   Func-count     min f(x)');
   fprintf(arq_iter,'\n %5.0f        %5.0f     %12.6g\n', itercount, func_evals, fv(1));
% Fim da Modificacao   
elseif prnt == 4
   clc
...
while func_evals < maxfun && itercount < maxiter
   ...
   %FAB - Troca de '' por algo que reflita melhor o estado do algoritmo.
   how = 'SEM_VALOR';
...
% Modificacao do Arquivo Original para guardar informacoes do percurso do
% algoritmo: 11/04/2006
   fprintf(arq_opt,' %5.0f        %5.0f     %12.6g         %s\n', itercount, func_evals, fv(1), how);
   fprintf(arq_iter,' %5.0f        %5.0f     %12.6g\n', itercount, func_evals, fv(1));
% Fim da Modificacao   
   elseif prnt == 4
      disp(' ')
      disp(how)
...
if func_evals >= maxfun 
   if prnt > 0
      ...
      %FAB - Otimização de código.
      %msg = sprintf('         Current function value: %f \n', fval);
      %disp(msg)
      fprintf('         Current function value: %f \n\n', fval);
% Modificacao do Arquivo Original para guardar informacoes do percurso do
% algoritmo: 13/04/2006
      fprintf(arq_opt,'\nExiting: Maximum number of function evaluations has been exceeded\n');
      fprintf(arq_opt,'         - increase MaxFunEvals option.');
      fprintf(arq_opt,'         Current function value: %f \n', fval);
% Fim da Modificacao   
   end
   exitflag = 0;
...
% algoritmo: 13/04/2006
      ...
      fprintf(arq_opt,'         - increase MaxIter option.');
      fprintf(arq_opt,'         Current function value: %f \n', fval);
% Fim da Modificacao   
   end
   exitflag = 0; 
...
% algoritmo: 13/04/2006
      fprintf(arq_opt,'\nOptimization terminated successfully:\n');
      fprintf(arq_opt,' the current x satisfies the termination criteria using OPTIONS.TolX of %e \n',tolx);
      fprintf(arq_opt,' and F(X) satisfies the convergence criteria using OPTIONS.TolFun of %e \n',tolf);
% Fim da Modificacao   
   end
   exitflag = 1;
end
