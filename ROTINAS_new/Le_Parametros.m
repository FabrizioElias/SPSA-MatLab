...
[fidinc1, msg] = fopen(DADOS.filepar1, 'r');%Abre o arquivo com valores iniciais de:bw e h (viga)
[fidinc2, msg] = fopen(DADOS.filepar2, 'r');%Abre o arquivo com valores iniciais de:hpx e hpy (pilar)
[fidinc3, msg] = fopen(DADOS.filepar3, 'r');%Abre o arquivo com valores iniciais de:hl (laje)
[fidinc4, msg] = fopen(DADOS.filepar4, 'r');%Abre o arquivo com o número de variáveis de projeto
[fidinc5, msg] = fopen(DADOS.filepar5, 'r');%Abre o arquivo com os números das variáveis de projeto
...
while ~feof(fidinc2)
        ...
        [Xpilar,count]=fscanf(fidinc2,'%f',[DADOS.Npilares inf])
        %Xpilar:matriz.
...
while ~feof(fidinc4)
        ...
        [Xotimiz,count]=fscanf(fidinc4,'%f',[3 inf]);
        break
end
...
while ~feof(fidinc5)  
        ...
        if VARPROJ.NvigasOT~=0
            [Xotv,count]=fscanf(fidinc5,'%f',[1,VARPROJ.NvigasOT]);
        else
            lixo=fgets(fidinc5);   
        end
        if VARPROJ.NpilaresOT~=0
            [Xotp,count]=fscanf(fidinc5,'%f',[1,VARPROJ.NpilaresOT]);
        end
        break
end
